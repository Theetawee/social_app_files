...
window.addEventListener("load", function () {
  ...
  function initialiseState(reg) {
    ...
    if (!reg.showNotification) {
      ...
      showMessage(
        gettext("Showing notifications are not supported in your browser.")
      );
      return;
    }
      ...
      if (subscription) {
        postSubscribeObj("subscribe", subscription, function (response) {
          ...
          if (response.status === 201) {
            ...
            isPushEnabled = true;
            showMessage(
              gettext("Successfully subscribed to notifications.")
            );
          }
        });
      }
      ...
      if (response.status === 202) {
        ...
        subscription
          ...
          .then(function (successful) {
            subBtn.textContent = gettext("Subscribe to Push Messaging");
            showMessage(
              gettext("Successfully unsubscribed from push notifications.")
            );
            isPushEnabled = false;
            subBtn.disabled = false;
          })
          .catch(function (error) {
            subBtn.textContent = gettext("Unsubscribe from Push Messaging");
            showMessage(
              gettext("Error while unsubscribing from push notifications.")
            );
            subBtn.disabled = false;
          });
      }
